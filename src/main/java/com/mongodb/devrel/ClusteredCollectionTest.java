...
public class ClusteredCollectionTest extends BaseMongoTest {
  ...
  public void run() {
    ...
    for (int op = 0; op < nOps; op++) {
      ...
      if (BasePriceDoc == null) {
        logger.error("Could not find base price for " + query);
      }
      ...
      for (int sp = 0; sp < nSellingPrices; sp++) {
        ...
        Bson updateSellingPriceKey =
            and(
                eq("cruiseCode", vals.get("cruiseCode")),
        ...
        Bson updateSellingPrice =
            combine(
                set("lastModified", Date.from(Instant.now())),
                set("deleted", false),
        ...
        if (a.getMatchedCount() == 0) {
          logger.error(
              "Could not update selling price for "
                  + updateSellingPriceKey.toBsonDocument().toJson());
        }
      }
    }
  }
  ...
  public void GenerateData() {
    ...
    Document basePriceIndex = new Document().append("cruiseCode", 1).append("basePriceUUID", 1);
  ...
  private void CreateSampleDate(
      MongoCollection<Document> collection, int[] docIds, int loadThreads, int threadNo) {
  ...
  Document generateRecord(int bpid, int variant, Random rng) {
    ...
    record.put("effectiveStartDate", Instant.now());
    record.put("effectiveEndDate", Instant.now().plus(30, ChronoUnit.DAYS));
