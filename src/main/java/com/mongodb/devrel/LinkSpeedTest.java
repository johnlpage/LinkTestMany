...
public class LinkSpeedTest extends BaseMongoTest {
  ...
  public void run() {
    ...
    for (int o = 0; o < nOps; o++) {
      ...
      // Same collection
      Bson fetchJoined =
          lookup(coll_one.getNamespace().getCollectionName(), parts[0], parts[1], "joined");
      ...
      if (this.threadNo == 0 && o == 0) {
        ...
        for (Bson b : pipeline) {
          logger.info(b.toBsonDocument().toJson());
        }
        logger.info("Testing " + nOps + " calls like " + rval.toJson());
        logger.info(
            "Data Return Size (MB) : "
                + (((long) rval.toJson().length() * (long) nTests) / (1024 * 1024)));
      }
    }
  }
  ...
  public void WarmCache() {
    ...
    rval = coll_two.aggregate(List.of(touch)).first();
